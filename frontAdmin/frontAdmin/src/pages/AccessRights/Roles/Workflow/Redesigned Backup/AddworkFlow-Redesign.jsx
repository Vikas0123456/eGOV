...
const AddWorkflow = () => {
    ...
    useEffect(() => {
        stateRef.current = {
            nodes,
            edges,
            workflowDataConfig: workflowDataConfig?.tableConfig,
            workflowID,
            userData,
        };
    }, [nodes, edges, workflowDataConfig?.tableConfig, workflowID, userData]);
    ...
    const debouncedUpdateStore = useCallback(
        debounce((updatedNodes) => {
            ...
            const newUpdatedData = workflowDataConfig.some(
                (config) => config.workflowID === workflowID
            )
                ? workflowDataConfig.map((config) =>
                      config.workflowID === workflowID
                          ? updatedWorkflowData
                          : config
                  )
                : [...workflowDataConfig, updatedWorkflowData];
        ...
        () => [
            {
                ...
                data: {
                    ...
                    workflowFor:
                        intValue?.workflowFor === "1" ? "Service" : "Ticket",
                    departmentName: intValue?.departmentId
                        ? departments?.find(
                              (dept) => dept.id === intValue?.departmentId
                          )?.departmentName
                        : "",
                    onClick: () => {
    ...
    const generateWorkflowNodes = useCallback(
        (workflowIndex) => {
            const xPos = baseConfig.initialX + baseConfig.horizontalSpacing;
            const yPos =
                baseConfig.initialY +
                workflowIndex * baseConfig.verticalSpacing;
            ...
            return [
                {
                    ...
                    data: {
                        ...
                        services:
                            intValue?.workflow?.[
                                workflowIndex
                            ]?.serviceListApproval?.join(", ") ||
                            "No services available",
                        Users:
                            intValue?.workflow?.[workflowIndex]
                                ?.workflowMethod === "user" &&
    ...
    const baseNodes = useMemo(() => getBaseNodes(), [getBaseNodes]);

    const { nodes: memoizedNodes, edges: memoizedEdges } = useMemo(() => {
        if (workflowDataConfigById) {
            return {
                nodes: workflowDataConfigById?.nodes?.map((node, index) => ({
                    ...node,
                    data: {
                        ...node?.data,
                        onClick: () => {
                            setShow(true);
                            setCurrentworkflow(index === 0 ? null : index - 1);
                        },
                    },
                })),
                edges: workflowDataConfigById.edges,
            };
        } else {
            let workflowNodes = [];
            let workflowEdges = [];
            ...
            intValue?.workflow?.forEach((_, index) => {
                ...
                workflowNodes = [...workflowNodes, ...newNodes];

                // Create edge from previous node
                const targetNode = newNodes[0];
                const sourceNode =
                    index === 0 ? baseNodes[0] : workflowNodes[index - 1];
            ...
            // Combine base and workflow nodes
            return {
                nodes: [...baseNodes, ...workflowNodes],
                edges: workflowEdges,
            };
        }
    }, [
        workflowDataConfigById,
        intValue?.workflow,
        baseNodes,
    ...
    useEffect(() => {
        if (tableConfigLoading) return;
        setNodes(memoizedNodes);
        setEdges(memoizedEdges);
    }, [memoizedNodes, memoizedEdges, tableConfigLoading]);
