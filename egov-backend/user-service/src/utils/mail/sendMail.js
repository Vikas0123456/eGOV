...
module.exports = {
  forgotPassword: async (email, htmlContent) => {
  ...
  sendOTPmail: async (email, htmlContent) => {
    // email.tst
        let setting = await generalsettingModal.findAll({raw: true});
        const extractedSettings = setting.reduce((acc, setting) => {
            acc[setting.settingKey] = setting.settingValue;
    ...
    const emailParams = {
      EmailTo: toEmail || "", // Define To Email Address and please note that if multiple email addresses then pass comma separated.
      Subject: "OTP", // Define Email Subject
  ...
  emailConfirmationmail: async (email, htmlContent) => {
        let setting = await generalsettingModal.findAll({raw: true});
        const extractedSettings = setting.reduce((acc, setting) => {
            acc[setting.settingKey] = setting.settingValue;
            return acc;
    ...
    const emailParams = {
      ...
      // Message: `Welcome your Email is confirmed on Egov Platform!`,
      Message: `
      <!DOCTYPE html>
      <html>
  ...
  setPasswordwelcomemail: async (email, htmlContent) => {
        let setting = await generalsettingModal.findAll({raw: true});
        const extractedSettings = setting.reduce((acc, setting) => {
            acc[setting.settingKey] = setting.settingValue;
            return acc;
    ...
    const emailParams = {
      ...
      // Message: `<b> Set password with EGov Link </b>  <a href=http://localhost:3000/auth-pass-reset-cover/${userId} + '">click here</a>`,
      Message: `
      <!DOCTYPE html>
      <html>
  ...
  supportMail: async (email, htmlContent) => {
        let setting = await generalsettingModal.findAll({raw: true});
        const extractedSettings = setting.reduce((acc, setting) => {
            acc[setting.settingKey] = setting.settingValue;
            return acc;
    ...
    //     <p>Rating: ${customerData.rating}</p>
    //     <p>Note: ${customerData.note}</p>
    //     <footer>Customer ID: ${customerData.customerId}</footer>
    // `;

    const emailParams = {
    EmailTo: email || "",
    Subject: "Request for need support for customer",
    Message: `
    <!DOCTYPE html>
    <html>
  ...
  deleteAccountConfirmation: async (email, htmlContent) => {
    let setting = await generalsettingModal.findAll({raw: true});
    const extractedSettings = setting.reduce((acc, setting) => {
        acc[setting.settingKey] = setting.settingValue;
        return acc;
...
const emailParams = {
  EmailTo: toEmail || "", // Define To Email Address and please note that if multiple email addresses then pass comma separated.
  Subject: "Delete Account Request", // Define Email Subject
  // Message: `<b>Reset Password Link </b>  <a href=http://localhost:3000/auth-pass-reset-cover/${userId} + '">click here</a>`,
  Message: `
  <!DOCTYPE html>
  <html>
