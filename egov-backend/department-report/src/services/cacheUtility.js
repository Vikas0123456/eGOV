...
const fetchDataWithCaching = async (cacheFile, apiUrl, requestData = {},forceUpdate=false) => {
    try {
        ...
        if (forceUpdate) {
            writeCacheFile(cacheFile, data, (err) => {
                if (err) {
...
// const fetchUserData = (forceUpdate = false) => fetchDataWithCaching("user", `${process.env.USERSERVICE}internalCommunicationUser/getAlluser`, {}, forceUpdate);
