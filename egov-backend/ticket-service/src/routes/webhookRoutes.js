...
const {fetchDepartmentData,fetchServiceData,fetchDocumentData, fetchUserData, fetchCustomerData, fetchDocumentViewData} = require('../services/cacheUtility');

route.post('/webhook',async (req, res) => {
    const { changedEntity } = req.body;
    // console.log(changedEntity)
    try {
        ...
        // else if(changedEntity === 'fileupload'){
        //     // cacheQueue.enqueue(async () =>await fetchDocumentData(true));
        ...
        else if(changedEntity === 'customer'  ){
            // cacheQueue.enqueue(async () =>await fetchCustomerData(true))
